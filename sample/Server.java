...
public class Server implements ProjectLib.CommitServing {
    ...
    */
    private synchronized int generateCommitID() {
        commitID++;
        return commitID;
    } 
    ...
    */
    public void startCommit( String filename, byte[] img, String[] sources ) {
        ...
        Commit commitRunnable = new Commit(filename, img, sources, commitID);
    ...
    */
    ...
    public static class Commit implements Runnable {
        ...
        public Commit(String filename, byte[] img, String[] sources, int commitID) {
        ...
        public Commit(String filename, String[] sources, int commitID, byte[] img, String mode) {
            this.filename = filename;
            this.img = img;
            this.commitID = commitID;
            this.userImageMap = getSourcesByNodeMap(sources);
        ...
        */
        ...
        /**
        * Distributes failure decisions to all user nodes.
        */
        private void declareFailure() {
            ...
            for (String userNode: userNodes) {
                ...
                RequestResponseMessage requestResponseMessage  = new RequestResponseMessage(commitID, userNode, nodeSourcesArray, filename,
                                                                                            this.img, MessageType.COMMIT_FAILURE);
        ...
        /**
        * Distributes successful commit decisions to all user nodes.
        */
        private void distributeDecisions() {
            ...
            for (String userNode: userNodes) {
                ...
                RequestResponseMessage requestResponseMessage  = new RequestResponseMessage(this.commitID, userNode, nodeSourcesArray, filename,
                                                                                            this.img, MessageType.DISTRIBUTE);
                byte[] serializedBytes = serializeMessage(requestResponseMessage);
                PL.sendMessage(new ProjectLib.Message(userNode, serializedBytes));
    ...
    * @see RequestResponseMessage
    */
    public static void startCommitRecovery(String fileName) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
            ...
            if (isSuccess) {
                commitSuccessRunnable = new Commit(collageName, sources, commitID, img, "DISTRIBUTE");
                thread = new Thread(commitSuccessRunnable);
                thread.start();

            } else {
                // If the commit before the crash is a failure, start failure thread.
                commitFailureRunnable = new Commit(collageName, sources, commitID, img, "COMMIT_FAILURE");
